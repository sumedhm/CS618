...
void update_parent(int key, int sibling, stack<int>& parent){
	...
	bool inserted = false;
	for(int i=2;i<newbuff.size();i+=2){
		if(newbuff[i]>key){
			inserted = true;
...
void insert(int node, int val, string object, stack<int>& parent){
	...
	if (buff[0]){	//leaf node.
		...
		flag = 0;
		seek = 4;	//Leaving 4 spaces. 1 for isleaf, 2 for prev, next, 1 waste.
		while(1){
		...
		seek = 2; //1 for isleaf, 1 for 1st child pointer.
		parent.push(node);
